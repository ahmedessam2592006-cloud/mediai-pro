    ...
    if (stream) {
      ...
      try {
        ...
        if (pollRes.ok && pollRes.body) {
          ...
          // Handle remaining buffer - might be non-streaming JSON
          ...
          // If no text was sent, fall through to Gemini
        }
      } catch (pollErr) {
...
function convertGeminiToOpenAI(contents) {
  ...
  return contents.map(item => {
    ...
    // Convert first system-like user message to system role for better compatibility
