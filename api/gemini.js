export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(500).json({ 
      ...
      note: 'If this is a source-related error, the AI may have received sources but failed to process them. Check that sources include valid abstracts.'
    });
  }
}
...
function convertGeminiToOpenAI(contents) {
  ...
  for (const item of contents) {
    ...
    // Detect system prompts (strict source instructions, role definitions)
    if (role === 'user' && (
      text.includes('You are MediAI Pro') || 
      text.includes('STRICT evidence-based') ||
      text.includes('CRITICAL RULES') ||
      text.includes('VERIFIED MEDICAL SOURCES')
    )) {
      // This is a system prompt disguised as user message
  ...
  // Prepend system message if found
  if (systemContent) {
    messages.unshift({
      role: 'system',
