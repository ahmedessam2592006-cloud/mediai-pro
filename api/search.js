export default async function handler(req, res) {
  ...
  try {
        ...
        (async () => {
          try {
            let searchUrl = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=' + 
              encodeURIComponent(cleanQuery) + '&retmax=6&retmode=json&sort=date';
        ...
        (async () => {
          try {
            ...
            const url = 'https://www.ebi.ac.uk/europepmc/webservices/rest/SEARCH?query=' + 
              ...
              '&pageSize=6&format=json&resultType=core&sort_date=y';
      ...
      (async () => {
        try {
          ...
          const url = 'https://api.semanticscholar.org/graph/v1/paper/search?query=' + 
            encodeURIComponent(cleanQuery) + 
            '&fields=title,authors,year,externalIds,abstract,journal,publicationDate,paperId&limit=6';
          const r = await fetch(url, { headers: { Accept: 'application/json' }, signal: controller.signal });
          clearTimeout(timeout);
          if (!r.ok) return [];
          const data = await r.json();
    ...
    unique.forEach(paper => {
      ...
      const trustScore = priority === 1 ? 50 :  // PubMed
                        priority === 2 ? 35 :  // Europe PMC
                        priority === 3 ? 20 : 0; // Semantic Scholar
    ...
    // Take top 12, ensuring diversity
    const final = [];
    const sourceCounts = {};
    ...
    for (const paper of unique) {
      ...
      if (sourceCounts[src] < maxPerSource && final.length < 12) {
        final.push(paper);
        sourceCounts[src]++;
      }
    }
