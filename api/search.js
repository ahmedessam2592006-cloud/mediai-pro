        ...
        (async () => {
          try {
            ...
            return data.resultList.result.map(paper => {
              ...
              const link = buildSourceLink({ doi, pmid });
              return {
                ...
                doi, pmid, abstract: paper.abstractText || '', url: link, source: 'Europe PMC'
              };
            }).filter(p => p.url && p.title !== 'Unknown Title');
          } catch (e) { return []; }
        })()
      );
    }
            ...
            for (const id of searchData.esearchresult.idlist.slice(0, 10)) {
              ...
              if (article?.title) {
                ...
                papers.push({
                  ...
                  doi, pmid, abstract: '', url: link, source: 'PubMed'
                });
              }
            }
            return papers;
      ...
      (async () => {
        try {
          ...
          const url = 'https://api.semanticscholar.org/graph/v1/paper/search?query=' + 
            encodeURIComponent(cleanQuery) + 
            '&fields=title,authors,year,externalIds,abstract,journal,publicationDate&limit=10';
          const r = await fetch(url, { headers: { Accept: 'application/json' }, signal: controller.signal });
          clearTimeout(timeout);
          if (!r.ok) return [];
          const data = await r.json();
          ...
          return data.data.map(paper => {
            ...
            return {
              ...
              doi, pmid, abstract: paper.abstract || '', url: link, source: 'Semantic Scholar'
            };
          }).filter(p => p.url && p.title !== 'Unknown Title');
        } catch (e) { return []; }
      })()
      ...
      (async () => {
        try {
          ...
          return data.message.items.map(item => {
            ...
            const link = buildSourceLink({ doi });
            ...
            return {
              ...
              year, doi, pmid: null, abstract: '', url: link, source: 'CrossRef'
            };
          }).filter(p => p.url && p.title !== 'Unknown Title');
        } catch (e) { return []; }
      })()
...
function buildSourceLink(paper) {
  const doi = validateDoi(paper.doi);
  const pmid = validatePmid(paper.pmid);
